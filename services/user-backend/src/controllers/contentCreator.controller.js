...
const contentService = require("../services/contentCreator.service");
...
function getPublicUserBase(req) {
  ...
  const normalizedBase =
    baseUrl.toLowerCase().endsWith("/content")
      ? baseUrl.slice(0, -"/content".length)
      : baseUrl;
...
async function uploadSceneImage(req, res) {
  try {
    ...
    return response(res, 201, "Scene image uploaded", {
      ...
      imageUrl: `${publicBase}/content/assets/${saved.fileName}`,
    });
  } catch (err) {
...
async function getVideoRenderJobStatus(req, res) {
  try {
    ...
    return response(res, 200, "Render job status", {
      ...
      videoUrl: job.status === "completed" ? `${publicBase}/content/video/${job.fileName}` : null,
    });
  } catch (err) {
