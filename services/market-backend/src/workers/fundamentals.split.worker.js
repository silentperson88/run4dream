...
const runtime = {
  section: readArg("section") || "profit_loss",
  masterId: readArg("master-id", "stock-id", "id"),
  symbol: readArg("symbol"),
  once: hasFlag("once", "single", "test"),
};

if (!SPLIT_TABLES[runtime.section]) {
  throw new Error(`Unknown section "${runtime.section}"`);
}
...
const processCandidate = async (master, snapshot) => {
  ...
  const result = await upsertSectionRows(runtime.section, master, snapshot, activeStockId, pool);
  return {
    label,
...
const run = async () => {
  ...
  console.log(
    `Starting fundamentals ${runtime.section} worker. once=${runtime.once}, masterId=${runtime.masterId || "all"}, symbol=${runtime.symbol || "all"}`,
  );

  const items = await loadCandidates();
  console.log(`${runtime.section} candidates loaded: ${items.length}`);
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    try {
      const { label, inserted } = await processCandidate(master, snapshot);
      success += 1;
      console.log(`[${i + 1}/${items.length}] OK ${label} | rows=${inserted}`);
    } catch (error) {
      failed += 1;
  ...
  console.log(`\n${runtime.section} worker completed.`);
  console.log({
    total: items.length,
    success,
    failed,
...
run()
  .catch((error) => {
    console.error(`Fatal fundamentals ${runtime.section} worker error:`, error?.message || error);
    process.exitCode = 1;
  })
  .finally(async () => {
    try {
