...
const {
  ensureActiveStockId,
  upsertPeriods,
} = require("../repositories/fundamentalsQuarterly.repository");
...
const processCandidate = async (master, snapshot) => {
  ...
  const result = await upsertPeriods(master, snapshot, activeStockId, pool);
...
const run = async () => {
  ...
  console.log(
    `Starting fundamentals quarterly worker. once=${runtime.once}, masterId=${runtime.masterId || "all"}, symbol=${runtime.symbol || "all"}`,
  );

  const items = await loadCandidates();
  console.log(`Quarterly candidates loaded: ${items.length}`);
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    try {
      ...
      success += 1;
      console.log(
        `[${i + 1}/${items.length}] OK ${label} | rows=${inserted}`,
      );
    } catch (error) {
      failed += 1;
      console.error(
...
run()
  .catch((error) => {
    console.error("Fatal quarterly worker error:", error?.message || error);
    process.exitCode = 1;
  })
  .finally(async () => {
    try {
